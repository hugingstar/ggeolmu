...
class DaskFinanceProcessor:
    ...
    def calculate_indicators(self, data):
        ...
        for r_idx in ["", "2", "3", "4", "5", "6", "7", "8", "9"]:
            ...
            # 일반 다이버전스
            bull, bear = self.divergence_optimized(df["MA2"], target_rsi, self.rsi_rollback)
            all_new_features[f"RSI_BullDiv{r_idx}"] = bull
            all_new_features[f"RSI_BearDiv{r_idx}"] = bear
            # 히든 다이버전스
            h_bull, h_bear = self.hidden_divergence_optimized(df["MA2"], target_rsi, self.rsi_hidden_rollback)
            all_new_features[f"RSI_Hidden_BullDiv{r_idx}"] = h_bull
            all_new_features[f"RSI_Hidden_BearDiv{r_idx}"] = h_bear
            # 일반 상승 추세
            up_trend, down_trend = self.trend_optimized(df["MA2"], target_rsi, self.rsi_trend_rollback)
